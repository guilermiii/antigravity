...
export default function AuthButtons({ compact = false }) {
  ...
  return (
    <div className={`auth-buttons-group ${compact ? 'compact' : ''}`}>
      <button
        type="button"
      ...
      >
        ...
        <span>{compact ? 'GitHub' : 'Entrar com GitHub'}</span>
      </button>
      ...
      >
        <GoogleIcon size={compact ? 16 : 18} />
        <span>{compact ? 'Google' : 'Entrar com Google'}</span>
      </button>
    </div>
  );
}
